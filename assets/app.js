(function () {
  ...
  function progressMetric(kind, id = "") {
    ...
    return `<span class="progress-metric" data-progress-kind="${kind}" data-progress-id="${escapeHtml(id)}"><span class="progress-ring" style="--progress:${percent}%" role="progressbar" aria-label="学习进度" aria-valuenow="${done}" aria-valuemin="0" aria-valuemax="${total}">${percent}%</span><span class="progress-fraction">${done}/${total} ${unit}</span></span>`;
  }
